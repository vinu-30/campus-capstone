...
exports.getStudentByEmail = asyncHandler(async (req, res) => { if (!req.query.email) return res.status(400).json({ success: false, message: 'email is required.' }); const student = (await Student.getAll()).find((item) => item.email.toLowerCase() === String(req.query.email).toLowerCase()); if (!student) return res.status(404).json({ success: false, message: 'Student profile not found.' }); res.json({ success: true, data: student }); });
